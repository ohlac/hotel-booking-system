...
// hjälpfunktion för att bokningsdatum inte ska kunna vara i det förflutna eller att check-out är innan check-in
function getTodayDateString() {
  return new Date().toISOString().slice(0, 10);
}

function isValidBookingDateRange(startDate, endDate) {
...
app.post("/api/create-checkout-session", async (req, res) => {
  ...
  const dateValidation = isValidBookingDateRange(startDate, endDate);
  ...
  try {
    ...
    if (conflicts.length > 0) {
      return res
        .status(409)
        .json({ message: "This room is no longer available for those dates." });
    }

    const millisecondsPerDay = 1000 * 60 * 60 * 24;
    const nights = Math.round(
      (new Date(endDate) - new Date(startDate)) / millisecondsPerDay,
    );
...
app.get("/api/confirm-booking", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      checkoutSession.metadata.userId !== String(req.session.user.id)
    ) {
    ...
    const { roomId, roomNumber, roomType, startDate, endDate } =
      checkoutSession.metadata;
  ...
  try {
    ...
    if (email) {
      const [emailRows] = await pool
        .promise()
        .query("SELECT id FROM users WHERE LOWER(email) = ? LIMIT 1", [email]);
      result.emailExists = emailRows.length > 0;
    }

    if (username) {
      const [usernameRows] = await pool
        .promise()
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "User was registered, but registration email failed:",
        emailError,
      );
    }
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    ...
    const frontendUrl =
      process.env.FRONTEND_URL || "https://hotel-frontend-vi9g.onrender.com";
